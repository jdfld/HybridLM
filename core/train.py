...
def main(_):
  ...
  try:
    ...
    # IterableDataset (streaming_tokenization=True) does not implement __len__.
    # Budget enforcement relies on the step > steps_budget check in the loop.
    pass
  ...
  for micro_step, micro_batch in enumerate(trainloader, micro_step_start + 1):
    ...
    # Eval
    valid_loss = None
    if cfg.eval and validloader is not None and step % cfg.eval_every_steps == 0 and is_step:
