...
```
python prepare.py \
  --out_path=/fast/jlindqvist/data/lm/fwedu/fwedu_sample_10B_tokenizer_GPT2 \
...
flags.DEFINE_string('out_path', '/fast/jlindqvist/data/lm/fwedu/test', 'Path where to save the dataset.')
...
flags.DEFINE_string('tokenizer', 'llama2', 'A valid tokenizer for transformers.AutoTokenizer.')
...
flags.DEFINE_integer('num_proc', None, 'Number of worker processes for map(). Defaults to os.cpu_count().')
flags.DEFINE_integer('map_batch_size', 4096, 'Batch size passed to dataset.map(). Larger values reduce Python overhead per batch.')
...
def main(_):
  ...
  num_proc = FLAGS.num_proc if FLAGS.num_proc is not None else os.cpu_count()
  map_setup = dict(batched=True, batch_size=FLAGS.map_batch_size, num_proc=num_proc)
  print(f'Using {num_proc} worker processes, map batch size {FLAGS.map_batch_size}.')

  # --------------------------------------------------------------------
  ## Pre-flight: check all output paths before any heavy work begins.

  if FLAGS.download and FLAGS.save_raw:
    raw_ds_path = os.path.join(out_path, 'raw_dataset')
    if os.path.exists(raw_ds_path):
      raise FileExistsError(f'Raw dataset already exists at {raw_ds_path}.')

  if FLAGS.tokenize and FLAGS.save_tokenized:
    tok_ds_path = os.path.join(out_path, f'tokenized_{tokenizer_name}', 'tokenized_dataset')
    if os.path.exists(tok_ds_path):
      raise FileExistsError(f'Tokenized dataset already exists at {tok_ds_path}.')

  if FLAGS.tokenize and FLAGS.save_tokenizer:
    tok_path = os.path.join(out_path, f'tokenized_{tokenizer_name}', 'tokenizer')
    if os.path.exists(tok_path):
      raise FileExistsError(f'Tokenizer already exists at {tok_path}.')

  if FLAGS.chunk and FLAGS.split_train_valid:
    ...
    if os.path.exists(train_ds_path):
      raise FileExistsError(f'Trainset already exists at {train_ds_path}.')
    if os.path.exists(valid_ds_path):
      raise FileExistsError(f'Validset already exists at {valid_ds_path}.')
  ...
  if FLAGS.tokenize:
    ...
    if FLAGS.save_tokenized:
      ...
      tokenized_ds.save_to_disk(os.path.join(out_path, f'tokenized_{tokenizer_name}', 'tokenized_dataset'), num_proc = num_proc)

    if FLAGS.save_tokenizer:
      print('Saving Tokenizer')
      tokenizer.save_pretrained(os.path.join(out_path, f'tokenized_{tokenizer_name}', 'tokenizer'), num_proc = num_proc)
  ...
  if not FLAGS.split_train_valid:
    ...
    # We do not shuffle before chunking, to avoid multiple documents contamination.
    ...
    train_ds.save_to_disk(train_ds_path, num_proc=num_proc)
    print('Saving validset')
    valid_ds.save_to_disk(valid_ds_path, num_proc=num_proc)
