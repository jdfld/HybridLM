...
def docs_lengths_to_cu_seqlens(docs_lengths: list[int], device='cpu') -> torch.Tensor:
  """
  Convert a single sequence's document-length list to FLA's cu_seqlens format.

  ``docs_lengths`` records how many tokens each document (or document fragment)
  occupies inside one packed sequence, e.g. ``[10, 20]`` for a sequence that
  contains a 10-token document followed by a 20-token document.

  FLA kernels (chunk_delta_rule, chunk_gla, chunk_rwkv7, …) accept a 1-D
  int32 tensor of *cumulative* offsets — one entry per document boundary plus
  a leading zero.  For the example above the result is ``[0, 10, 30]``.

  The tensor can be passed directly as the ``cu_seqlens`` keyword argument to
  any FLA layer or op.  FLA requires ``batch_size == 1`` when cu_seqlens is
  used, so this function is intended for the single-sequence case where the
  whole batch dimension has been collapsed into the sequence dimension.

  Args:
      docs_lengths: List of token counts for each document (or fragment) in
                    the packed sequence.  Must be non-empty and all values
                    must be positive.
      device:       Target device for the returned tensor (default: 'cpu').

  Returns:
      1-D int32 tensor of shape ``(len(docs_lengths) + 1,)`` with values
      ``[0, docs_lengths[0], docs_lengths[0]+docs_lengths[1], …, sum(docs_lengths)]``.

  Example::

      >>> docs_lengths_to_cu_seqlens([10, 20])
      tensor([ 0, 10, 30], dtype=torch.int32)
  """
  offsets = torch.zeros(len(docs_lengths) + 1, dtype=torch.int32, device=device)
  torch.cumsum(torch.tensor(docs_lengths, dtype=torch.int32, device=device), dim=0, out=offsets[1:])
  return offsets
...
def intra_doc_causal_mask(doc_boundaries: list, max_seq_length: int, device='cpu') -> torch.Tensor:
  ...
  for segment_length in doc_boundaries:
    segment_causal_mask_bool = torch.tril(torch.ones((segment_length, segment_length), dtype=torch.bool, device=device))
