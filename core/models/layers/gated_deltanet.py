...
class GatedDeltaNet(nn.Module):
    ...
    def __init__(
        ...
        use_beta: bool = True, # The update rules should be the only difference for fair comparisons
        use_D: bool = False, # so no d as the other don't have it
        layer_idx: int = None,
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if self.use_short_conv:
            ...
            q_conv, conv_state_q = self.q_conv1d(x=self.q_proj(hidden_states),
                                            mask=conv_mask,
                                            cache=conv_state_q,
                                            output_final_state=use_cache,
                                            seq_idx=seq_idx)
            k_conv, conv_state_k = self.k_conv1d(x=self.k_proj(hidden_states),
                                            mask=conv_mask,
                                            cache=conv_state_k,
                                            output_final_state=use_cache,
                                            seq_idx=seq_idx)
            v_conv, conv_state_v = self.v_conv1d(x=self.v_proj(hidden_states),
                                            mask=conv_mask,
                                            cache=conv_state_v,
                                            output_final_state=use_cache,
                                            seq_idx=seq_idx)
